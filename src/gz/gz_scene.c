                           ...
                           void *data)
{
    if (reason == MENU_CALLBACK_SWITCH_ON) {
        ...
        if (gz.guard_view_state != GUARDVIEW_INACTIVE)
            gz.guard_view_state = GUARDVIEW_STOPPING;
    }
    else if (reason == MENU_CALLBACK_THINK) {
...
struct menu *gz_scene_menu(void)
{
  ...
  menu_add_watch(&menu, 16, 13,
                 (uint32_t)&z64_game.room_ctxt.rooms[0].index, WATCH_TYPE_U8, 0, 0);
  menu_add_static(&menu, 0, 14, "no. rooms", 0xC0C0C0);
  menu_add_watch(&menu, 16, 14, (uint32_t)&z64_game.n_rooms, WATCH_TYPE_U8, 0, 0);
